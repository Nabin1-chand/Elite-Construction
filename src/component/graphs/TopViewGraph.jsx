...
const TopViewGraph = () => {
  const statusColors = {
    done: { bg: "rgba(34, 197, 94, 0.4)", border: "#22c55e", label: "Done" },
    active: { bg: "rgba(59, 130, 246, 0.4)", border: "#3b82f6", label: "Active" },
    critical: { bg: "rgba(239, 68, 68, 0.4)", border: "#ef4444", label: "Critical" },
  };
  ...
  const getStatus = (val) => {
    ...
    if (v === 'done' || v.startsWith('b01') || v.startsWith('b05') || v.startsWith('b06')) return 'done';
  ...
  return (
    <Box w="full" maxW="900px" mx="auto" p={4} bg="gray.50">
      <Box bg="white" p={8} borderRadius="2xl" boxShadow="xl">
        <Heading size="md" textAlign="center" mb={6}>Intersection Overlay Map</Heading>
        ...
        <Flex justify="center" align="flex-start" position="relative">
          ...
          {/* Y-Axis Numbers and Title */}
          <VStack spacing="34px" pt="10px" pr={6} position="relative" zIndex={2}>
            {yTicks.map(tick => (
              <Text key={tick} fontSize="xs" fontWeight="bold" color="gray.500">{tick}</Text>
            ))}
          ...
          <Box position="relative">
            {/* GRID LINES (X and Y Background Lines) */}
            <Box position="absolute" top="18px" left="18px" right="0" bottom="0" pointerEvents="none">
              {/* Vertical Lines */}
              {xTicks.map((_, i) => (
                <Box key={`vl-${i}`} position="absolute" left={`${i * 43.5}px`} top="0" bottom="0" w="1px" bg="gray.100" />
              ))}
              {/* Horizontal Lines */}
              {yTicks.map((_, i) => (
                <Box key={`hl-${i}`} position="absolute" top={`${i * 43}px`} left="0" right="0" h="1px" bg="gray.100" />
              ))}
            </Box>

            {/* THE CIRCLE GRID */}
            <VStack spacing="-12px" align="flex-start" position="relative" zIndex={1}>
              {gridData.map((row, rowIndex) => (
                <Flex key={rowIndex} ml={rowIndex % 2 === 0 ? "0px" : "25px"}> 
                  {row.map((item, colIndex) => {
                    const status = getStatus(item);
                    const colors = statusColors[status];
                    const isStatusText = ['done', 'critical', 'active'].includes(item.toLowerCase());
                    ...
                    return (
                      <Circle
                        ...
                        transition="all 0.2s"
                        _hover={{ zIndex: 10, transform: "scale(1.2)", cursor: "pointer" }}
                      >
                        <Text fontSize="9px" fontWeight="black" color="black" pointerEvents="none">
                          {isStatusText ? "" : item}
                        </Text>
                      </Circle>
                    );
            ...
            <Box mt={6} position="relative" zIndex={2}>
              ...
              <Center mt={4}>
                <Text fontSize="xs" fontWeight="black">X (m)</Text>
              </Center>
            </Box>
          </Box>
